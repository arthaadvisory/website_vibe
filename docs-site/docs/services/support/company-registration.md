...
description: Comprehensive company incorporation and licensing services for domestic and international investors in Nepal.
icon: 🏢
...
```mermaid
graph TD
    A[Name Reservation - OCR] --> B[Drafting MOA/AOA]
    B --> C[Shareholder Resolution]
    C --> D[Company Incorporation - OCR]
    D --> E[PAN/VAT Registration - IRD]
    E --> F["SSF & Municipality Licensing"]
    F --> G[Industry Registration - DOI]
